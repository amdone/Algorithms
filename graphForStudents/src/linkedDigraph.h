...
#ifndef SRC_LINKEDDIGRAPH_H_
#define SRC_LINKEDDIGRAPH_H_
...
class linkedDigraph : public graph<bool>{
   ...
   public:
      ...
      void topologicalSort(int results[]);
      void bfs(int v, int reach[], int label);
      void rDFS(int vex, bool visited[], int results[], int& pos);
...
int linkedDigraph::degree(int theVertex) const{
 ...
 void linkedDigraph::rDFS(int vex,bool *visited,int *result,int &pos){
	 visited[vex] = true;
	 myIterator *v = iterator(vex);
	 int u;
	 while ((u = v->next()) != 0)
		 if (visited[u] == 0)
	     rDFS(u,visited,result,pos);
	 delete v;
	 result[pos++] = vex;
 }
 std::ostream& operator<<(std::ostream& out, const linkedDigraph& x){
...
void linkedDigraph::topologicalSort(int *result){
	bool visited[n];
	for(int i=1;i<n;i++)visited[i]=false;
	int pos = 0;
	for(int i=1;i<n;i++)if(!inDegree(i))rDFS(i,visited,result,pos);
	for(int i=0;i<n/2;i++){
		int temp = result[i];
