...
template<class T>
binaryTreeNode<std::pair<int, T> >* maxHblt<T>::meld(binaryTreeNode<std::pair<int, T> >* x, binaryTreeNode<std::pair<int, T> >* y)
{// Meld leftist trees with roots *x and *y. Return pointer to new root in x.
   ...
   binaryTreeNode<std::pair<int, T> >* result = meld(x->rightChild,y);
   x->rightChild = result;
...
inline void maxHblt<T>::meld(maxHblt<T>& theHblt){// meld *this and theHblt
	  binaryTreeNode<std::pair<int, T> >* result = meld(this->root, theHblt.root);
	  this->root = result;
