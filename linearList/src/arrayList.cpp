...
arrayList<T>::arrayList(int initialCapacity) {
	//因为是构造函数，arrayList对象还不存在
	//通过设置成员变量的值，构造一个新arrayList对象
	...
	//空线性表，但实际上已经生成了arrayLength个 T 的对象！！！
	//只是看作不存在
	listSize = 0;
}
...
arrayList<T>& arrayList<T>::operator=(const arrayList<T>& rhs){
	//=号左边的this和右边的rhs是已经存在的对象
	if(this == &rhs) return *this;//自己等于自己，例如 a=a
	listSize = rhs.listSize;

	//必须考虑左边的空间是不是足以容纳右边的所有数据
...
template<class T>
void arrayList<T>::changeLength1D(T * &source, int oldLength, int newLength){
	if(newLength < 0)
...
template<class T>
arrayList<T>& arrayList<T>::operator=(const chain<T>& rhs){

	//必须考虑左边的空间是不是足以容纳右边的所有数据
	if(arrayLength < rhs.size()){
		arrayLength = rhs.size();
		delete [] element;
		element = new T[arrayLength];
	};
	for(int i = 0;i< rhs.size();i++){
		this->insert(i,rhs.get(i));
	}
	return *this;
}
